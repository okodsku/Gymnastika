  ...
  <div class="modal-dialog modal-xl" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="studentModalLabel">Agregar Disciplina</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <div class="modal-body">
        <form action="" method="post" id="formDisciplina">
        <nav>
...
<script>
  $(document).ready(function(){
    $('#tablaDisciplina').load('Busquedas/TablaDisciplina.php');
    $('#formDisciplina').on('submit', function(e){
        e.preventDefault();

        $.ajax({
            url: 'Busquedas/AgregarDisciplina.php',
            type: 'post',
...
let eliminar = (id) => {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                url: 'Busquedas/EliminarDisciplina.php',
                type: 'post',
                data: {id: id},
                success: function(response){
                    Swal.fire(response, '', 'success');
...
let recordatorio = () => {
    Swal.fire({
        position: "top-center",
        title: '!Agrega La Nueva Disciplina!',
        icon: 'info',
